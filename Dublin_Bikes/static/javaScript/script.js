function myMap() {
    ...
    $.getJSON('/stations', null, function(data) {
        ...
        //code to dynamically fill drop down menu for stations in form for prediction
        var i;
        var stationHTML = "";
        ...
        //gets the se;ect from index.html and fills it with these options
        document.getElementById("predictionStation").innerHTML = stationHTML;

        //calls the function to make the dynamic dropdown for the days
        makeDayDropdown();
        ...
        if ('stations' in data) {
            ...
            _.forEach(stations, function(station) {
                ...
                //less than 10 available bikes its orange
                ...
                //as I add makers to the array it gives its index by the length of the current array -1. ie the first station has index. this code creates the stations names in the side bar of the map. and when theyre clicked it calls the myclick function with the index of the marker object
                ...
                marker.addListener("click", function() {
                    ...
                    //hides the directions if theyre shown
...
//this function is activated when the submit button is clicked on the prediction form
// it sends json data to the modelPredictions route in views.py via POST
// recieves back result from modelPredictions and then it renders it to the html
    ...
    $('#submit_button').click(function(event) {
        $.ajax({
            ...
            if (result.error) {
                ...
                document.getElementById('result').innerHTML ="<h3 id='predictedResult'>"+result+"</h3>";
...
//this function dynamically makes the day dropdown for the predictio form. The value are based on the current day values in the weather_prediction table in the database
...
//when the station name in the drop down is clicked this function is called. It accepts the index of the marker in the markers array and it triggers the click of a marker
function myclick(i) {
...
function dynamicStationData(marker) {
    //gets data from the station availabilty route
    $.getJSON('/station_availability/' + marker.station_number, null, function(data) {
        ...
        if ('stations' in data) {
            ...
            // this varible holds the dynamic data that will be displayed in the infoWindows 
            ...
            //adds a listener to the directions button and edits the display of the page
            document.getElementById("nodeGoto").addEventListener("click", function() {
                ...
                //pass to a directions function the marker
                Directions(marker);
...
//this accepts the marker as an argument
function Directions(m) {
    ...
    //these two lines show the directions and blue line when the show directions is clicked. They have been hidden when the marker is first clicked
...
//displays the weather info under the nav bar when weather is clicked
...
//used for when screen is resized
...
//shows corresponding charts depending when average daly demand button is clicked on application
...
//shows corresponding charts depending when average hourly demand button is clicked on application
...
//changes to predict where the user can make their predictions
...
var stations_window = false; //stations window for side bar drop down
